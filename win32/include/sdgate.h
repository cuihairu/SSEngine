...
public:    
    ...
    //
	virtual void SSAPI Connect(const char* pszIP, UINT16 wPort, bool bAutoReconnect) = 0;
    ...
    //
	virtual bool SSAPI Enter(ISSClientConnection* poClient, UINT32 dwTransID) = 0;

    //
    // Name     : Enter
    // Function : Before client close one server's connect, client should leave the server.
    //
	virtual bool SSAPI Leave(ISSClientConnection* poClient, UINT32 dwTransID) = 0;
...
public:
    ...
    //
	virtual void SSAPI OnEnter(ISSClientConnection* poClient, INT32 nSDError, UINT32 dwTransID) = 0;

    //
    // Name     : OnLeave
    // Function : Callback when leave the server that need to transmit to client.
    //
	virtual void SSAPI OnLeave(ISSClientConnection* poClient, INT32 nSDError, UINT32 dwTransID) = 0;
};
